...
B_scale = 1.3 # Typical field we would produce is on the order of 2.5T
B_max = B_scale*fullfield(0,0,10e-3,stagescale)   

z_position = 0.0e-3 #in units of mm
B_val = B_scale*fullfield(0,0,z_position,stagescale)*0.001
Bfield = np.linspace(B_val,B_val,1)

z_position2 = 0.1e-3 #in units of mm
...
eta = np.linspace(0,B_max,int(1e4))        # T
plt.figure(2)
plt.grid(True)
...
#t = ("???")
#plt.text(11, 10, t, fontsize=18, style='oblique', ha='center',rotation=15,va='top', wrap=True)
...
#%%
transition_amplitude(excited_eigenvectors.H,ground_eigenvectors.H,q=0)
#transition_amplitude(excited_eigenvectors2.H,ground_eigenvectors2.H,q=0)
